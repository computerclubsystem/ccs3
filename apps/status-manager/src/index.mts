...
import { createPingMessage } from '@computerclubsystem/types/messages/ping.mjs';

const messageBusIdentifier = 'ccs3/status-manager';
...
setInterval(async () => {
    ...
    const publishResult = await publishMessage(sharedChannel, pingMessage);
...
const publishMessage = async (channelName: ChannelName, message: Message): Promise<number> => {
    try {
        console.log('Publishing message', channelName, message.header.type);
        message.header.source = messageBusIdentifier;
        return await pubClient.publish(channelName, serializeMessage(message));
    } catch (err) {
